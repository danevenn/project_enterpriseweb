...
export function ProductCategoryForm({
  ...
  const {
    ...
    resolver: zodFormResolver<CreateProductCategoryInput>(createProductCategorySchema),
  ...
  async function onSubmit(values: CreateProductCategoryInput) {
    try {
      if (isEdit && category) {
      ...
      <DialogContent className="sm:max-w-md">
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="grid gap-4">
          <div className="grid gap-2">
            <Label htmlFor="pcat-name">Nombre</Label>
            <Input id="pcat-name" {...register("name")} />
            {errors.name && <p className="text-sm text-destructive">{errors.name.message}</p>}
          </div>
          <div className="grid gap-2">
            <Label htmlFor="pcat-desc">Descripción</Label>
            <Input id="pcat-desc" {...register("description")} />
            {errors.description && (
