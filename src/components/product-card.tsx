...
import { isLowStock, LOW_STOCK_THRESHOLD } from "@/lib/product-utils";
...
export function ProductCard({ product }: { product: ProductWithCategory }) {
  ...
  const lowStock = isLowStock(product, LOW_STOCK_THRESHOLD);
  ...
  return (
    <Card data-testid="product-card" className="flex flex-col transition-shadow hover:shadow-md">
      <CardHeader>
        ...
        {lowStock && !outOfStock && (
          <Badge variant="outline" data-testid="low-stock-badge" className="w-fit text-amber-600">
            Poco stock
          </Badge>
        )}
        {product.description && (
